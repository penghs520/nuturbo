...
public class Condition {
    ...
    public Condition(List<ConditionItem<?, ?>> ands, List<LogicCondition> logics) {
        this.ands = ands != null ? ands.stream().filter(Objects::nonNull).toList() : new ArrayList<>();
        this.logics = logics != null ? logics.stream().filter(Objects::nonNull).toList() : new ArrayList<>();
    }
