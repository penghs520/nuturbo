...
/**
 * 自定义动作
 * 设计初衷：希望通过自定义动作来封装一组内置动作（这组动作并不能原子化），不仅可以简化操作，还允许你设计自己的业务流程
 * 例如：给版本卡设置一个完成的动作，当点击完成时，将版本卡的状态设置为完成，同时设置版本的完成时间为当前时间
 */
@Getter
public class CustomAction extends Schema<ActionId> implements Action {
    ...
    protected CustomAction(ActionId id, OrgId orgId, String name, String description, CardTypeId cardTypeId, ConditionId permission, List<InternalAction> executeActions) {
